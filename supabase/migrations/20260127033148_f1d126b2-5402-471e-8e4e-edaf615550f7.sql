-- Drop and recreate the function to include owner in available technicians
...
BEGIN
  ...
  WHILE v_current_slot + (p_duration_minutes || ' minutes')::INTERVAL <= v_close_time LOOP
    ...
    -- Now includes: technicians AND owners (always available)
    SELECT 
      CASE 
    ...
    WHERE u.business_id = p_business_id
      ...
      -- Include technicians OR owners
      AND (u.role = 'technician' OR u.role = 'owner')
