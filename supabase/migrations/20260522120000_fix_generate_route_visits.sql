-- Fix generate_route_visits: customer address fallback, service seed, clearer errors
...
BEGIN
  ...
  LOOP
    ...
    -- CSV imports store address on customers table directly
  ...
  WHERE a.route_stop_id IN (
    SELECT id FROM public.route_stops WHERE route_id = p_route_id AND is_active = true
  )
    AND a.scheduled_date = p_scheduled_date
...
BEGIN
  v_user_business := public.get_user_business_id();
  IF v_user_business IS NULL THEN
    RAISE EXCEPTION 'Not signed in or business not set up';
  END IF;
  IF p_business_id IS DISTINCT FROM v_user_business THEN
    RAISE EXCEPTION 'Access denied';
  END IF;
