  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('businesses')
        .select('id, name, slug, phone, email, website, address, city, state, zip_code, country, timezone, logo_url, mapbox_public_token, settings, subscription_status, subscription_ends_at, created_at, updated_at')
        .eq('id', businessId)
        .single();
